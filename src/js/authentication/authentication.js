...
//     // event.preventDefault()
//     getUser().then(data => {
//         const a = !data.map(el => el.email.toLowerCase()).includes(signinEmail.value.toLowerCase())
//         const b = !data.map(el => el.password).includes(signinPassword.value)
//         signinEmail.classList.toggle('cd-signin-modal__input--has-error', a)
//         signinPassword.classList.toggle('cd-signin-modal__input--has-error', b)
//         if (!a && !b) {
//             LogedIn()
